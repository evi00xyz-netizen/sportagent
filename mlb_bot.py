...
class PitcherData(BaseModel):
    ...
    xfip: Optional[float] = Field(default=None, description="Expected Fielding Independent Pitching")
    siera: Optional[float] = Field(default=None, description="Skill-Interactive ERA")
    k_bb_pct: Optional[float] = Field(default=None, description="K-BB%")
    vs_lhb: Optional[float] = Field(default=None, description="xFIP vs left-handed batters")
    vs_rhb: Optional[float] = Field(default=None, description="xFIP vs right-handed batters")

class OffenseData(BaseModel):
    wrc_plus: Optional[float] = Field(default=None, description="Weighted Runs Created Plus vs handedness of opposing starter")
    iso: Optional[float] = Field(default=None, description="Isolated Power vs handedness of opposing starter")
    ops: Optional[float] = Field(default=None, description="OPS vs handedness of opposing starter")

class BullpenData(BaseModel):
    rested_key_relievers: Optional[int] = Field(default=None, description="Number of high-leverage relievers fully rested (<15 pitches last 72hrs)")
    aggregate_xfip: Optional[float] = Field(default=None, description="Bullpen aggregate xFIP")
    fatigue_flag: Optional[str] = Field(default=None, description="'fresh', 'moderate', 'taxed'")

class VenueData(BaseModel):
    park_factor: Optional[float] = Field(default=None, description="Overall park run factor (100 = neutral)")
    hr_factor: Optional[float] = Field(default=None, description="Home run park factor")
...
class SabermetricOutput(BaseModel):
    """LLM outputs ONLY these structural sabermetric variables. NO probabilities."""
    away_team: str = Field(...)
    home_team: str = Field(...)
...
def extract_mlb_markets_from_event(event: dict) -> list[dict]:
    ...
    MLB events typically have: Away win, Home win (no draw)."""
    ...
    for m in markets:
        ...
        # MLB: look for "win" outcomes — moneyline (no draw in baseball)
        teams = event.get("teams", [])
        away_name = ""
        home_name = ""
        if len(teams) >= 2:
            away_name = (teams[0].get("name") or "").lower()
            home_name = (teams[1].get("name") or "").lower()

        if "win" not in question:
            continue

        if away_name and away_name in question:
            market_type = "away"
        elif home_name and home_name in question:
            market_type = "home"
            ...
            # Fallback: first team mentioned = away
            if question.startswith(away_name) or away_name in question.split()[0:3]:
                market_type = "away"
            elif home_name and home_name in question:
                market_type = "home"
            else:
...
def extract_pitchers_from_event(event: dict) -> tuple:
    """Try to extract pitcher names from Gamma event data."""
    away_pitcher = "TBD"
    home_pitcher = "TBD"

    # Gamma may have pitcher data in markets or metadata
    markets = event.get("markets", [])
    for m in markets:
        question = m.get("question", "")
        # Look for pitcher names in market questions
        # Common format: "Pitcher Name over X strikeouts" etc.
        if "strikeout" in question.lower() or "pitch" in question.lower():
    ...
    if isinstance(meta, dict):
        away_pitcher = meta.get("awayPitcher") or meta.get("away_pitcher") or away_pitcher
        home_pitcher = meta.get("homePitcher") or meta.get("home_pitcher") or home_pitcher

    return away_pitcher, home_pitcher


def extract_stadium_from_event(event: dict) -> str:
    """Try to extract stadium/venue name from Gamma event data."""
    venue = event.get("venue") or event.get("stadium") or event.get("location")
    ...
    meta = event.get("metadata") or event.get("meta") or {}
    if isinstance(meta, dict):
        return meta.get("venue") or meta.get("stadium") or meta.get("location") or "Unknown"
...
async def fetch_sabermetric_variables(
    ...
    """
    LLM outputs isolated sabermetric variables ONLY.
    No probabilities — those are calculated downstream by Python.
    """
    ...
    # Parse sabermetric output — LLM outputs variables, NOT probabilities
...
# ALL probability math is done HERE, NOT by the LLM.

def sabermetrics_to_probabilities(sm: SabermetricOutput) -> dict:
    """
    Convert isolated sabermetric variables into win probabilities.
    Uses a log5-style model with adjustments for pitching, offense, bullpen, and park.

    This is the ONLY place probabilities are calculated — deterministic Python code.
    The LLM never touches probability math.
    """
    # ── Default values ──
    league_avg_xfip = 4.20          # 2024-2025 MLB average
    league_avg_wrc_plus = 100.0
    league_avg_iso = 0.155
    league_avg_k_bb = 14.0          # average K-BB%
    ...
    away_siera = sm.away_pitcher.siera or away_xfip
    home_siera = sm.home_pitcher.siera or home_xfip
    ...
    # ── Pitching component: lower xFIP = better ──
    # Normalize: (league_avg - pitcher_xfip) = positive when pitcher is above avg
    away_pitching_edge = (league_avg_xfip - home_xfip) * 0.08   # away pitcher vs home offense
    home_pitching_edge = (league_avg_xfip - away_xfip) * 0.08   # home pitcher vs away offense

    # K-BB% bonus: high K-BB% correlates with run prevention
    away_kbb_edge = ((away_kbb - league_avg_k_bb) / league_avg_k_bb) * 0.06
    home_kbb_edge = ((home_kbb - league_avg_k_bb) / league_avg_k_bb) * 0.06

    # ── Offense component: higher wRC+ = better ──
    ...
    # ISO bonus: power correlates with run scoring
    ...
    # Baseball home field advantage is ~54% (0.04 above neutral)
    ...
    # HR factor — affects power-heavy teams disproportionately
    hr_factor = sm.venue.hr_factor or 100.0
    hr_modifier = ((hr_factor - 100.0) / 100.0) * 0.02
    # Higher HR park = benefits team with higher ISO
    ...
    # Weather: wind blowing out = more offense, wind blowing in = less
    weather_modifier = 0.0
    wind_speed = sm.venue.wind_speed_mph or 0.0
    wind_dir = (sm.venue.wind_direction or "").lower()
    temp = sm.venue.temp_f or 72.0

    # Temperature: ball travels farther in warm air
    temp_modifier = ((temp - 72.0) / 72.0) * 0.015

    # Wind: approximate effect (simplified — depends on park orientation)
    if wind_speed > 8 and "out" in wind_dir:
        weather_modifier = wind_speed * 0.002  # blowing out = more offense
    elif wind_speed > 8 and "in" in wind_dir:
        weather_modifier = -wind_speed * 0.002  # blowing in = less offense

    # ── Aggregate edges ──
    # Away team's probability edge (vs neutral 50%)
    away_edge = (
        away_pitching_edge + away_kbb_edge
        + away_offense_edge + away_iso_edge
        + away_bullpen_edge + away_hr_edge
        - home_field_advantage - park_modifier
    )

    # Home team's edge
    home_edge = (
        home_pitching_edge + home_kbb_edge
        + home_offense_edge + home_iso_edge
        + home_bullpen_edge + home_hr_edge
        + home_field_advantage + park_modifier
    )

    # Weather modifier affects both teams equally — adds variance, not bias
    # (applied to overall run scoring environment, not individual probability)

    # ── Convert edges to probabilities using sigmoid ──
    # Baseline: assume 46% away / 54% home before adjustments
    base_away = 0.46
    base_home = 0.54

    # Apply edges through sigmoid to keep in [0,1]
    def sigmoid(x: float) -> float:
        return 1.0 / (1.0 + math.exp(-x))

    # Net edge: positive favors away, negative favors home
    net_edge = away_edge - home_edge
    away_prob = sigmoid(math.log(base_away / base_home) + net_edge * 3.0)
    home_prob = 1.0 - away_prob

    # Clamp to reasonable range [0.25, 0.75]
    ...
    if sm.away_pitcher.xfip and sm.home_pitcher.xfip:
        parts.append(
            f"Pitching: {sm.away_pitcher.name} xFIP {sm.away_pitcher.xfip:.2f}"
            f" vs {sm.home_pitcher.name} xFIP {sm.home_pitcher.xfip:.2f}"
        )
    if sm.away_offense.wrc_plus and sm.home_offense.wrc_plus:
        parts.append(
            f"Offense: {sm.away_team} wRC+ {sm.away_offense.wrc_plus:.0f}"
            f" vs {sm.home_team} wRC+ {sm.home_offense.wrc_plus:.0f}"
        )
    ...
    if sm.away_bullpen.fatigue_flag or sm.home_bullpen.fatigue_flag:
        parts.append(
            f"Bullpen: AWAY {sm.away_bullpen.fatigue_flag or 'normal'}"
            f" | HOME {sm.home_bullpen.fatigue_flag or 'normal'}"
        )
...
async def cmd_mlb(ctx, *, args: str = ""):
    """Analyze a Polymarket MLB event with sabermetric EV + Kelly staking."""
    ...
    try:
        ...
        if not slug:
            await ctx.send(f"❌ Could not extract event slug from URL.")
            return
        ...
        async with ctx.typing():
            ...
            # Extract pitchers and stadium from event
            away_pitcher, home_pitcher = extract_pitchers_from_event(event)
            stadium_name = extract_stadium_from_event(event)

            # ── Step 2: Extract MLB moneyline markets ──
            markets = extract_mlb_markets_from_event(event)
            print(f"[MLB CMD] Found {len(markets)} MLB markets", file=sys.stderr)
            if len(markets) < 2:
                await ctx.send(f"❌ Could not find enough MLB markets (away/home). Found: {len(markets)}.")
                return
            ...
            # ── Step 4: Sabermetric Engine (LLM outputs variables ONLY) ──
            ...
            print(f"[MLB CMD] Sabermetrics received: away_xFIP={sm.away_pitcher.xfip}, "
                  f"home_xFIP={sm.home_pitcher.xfip}, park_factor={sm.venue.park_factor}",
                  file=sys.stderr)

            # ── Step 5: Python calculates probabilities from sabermetrics ──
            probs = sabermetrics_to_probabilities(sm)
            print(f"[MLB CMD] Python probabilities: away={probs['away_prob']:.3f}, "
                  f"home={probs['home_prob']:.3f}", file=sys.stderr)
            ...
            if sm.away_pitcher.xfip and sm.home_pitcher.xfip:
                pitcher_lines.append(
                    f"**{sm.away_pitcher.name}** (AWAY): xFIP {sm.away_pitcher.xfip:.2f} | "
                    f"SIERA {sm.away_pitcher.siera:.2f}" if sm.away_pitcher.siera else ""
                    f" | K-BB% {sm.away_pitcher.k_bb_pct:.1f}%"
                )
                pitcher_lines.append(
                    f"**{sm.home_pitcher.name}** (HOME): xFIP {sm.home_pitcher.xfip:.2f} | "
                    f"SIERA {sm.home_pitcher.siera:.2f}" if sm.home_pitcher.siera else ""
                    f" | K-BB% {sm.home_pitcher.k_bb_pct:.1f}%"
                )
            else:
            ...
            if sm.away_offense.wrc_plus and sm.home_offense.wrc_plus:
                offense_lines.append(
                    f"**{sm.away_team}**: wRC+ {sm.away_offense.wrc_plus:.0f} | "
                    f"ISO {sm.away_offense.iso:.3f}" if sm.away_offense.iso else ""
                    f" | OPS {sm.away_offense.ops:.3f}" if sm.away_offense.ops else ""
                )
                offense_lines.append(
                    f"**{sm.home_team}**: wRC+ {sm.home_offense.wrc_plus:.0f} | "
                    f"ISO {sm.home_offense.iso:.3f}" if sm.home_offense.iso else ""
                    f" | OPS {sm.home_offense.ops:.3f}" if sm.home_offense.ops else ""
                )
            if offense_lines:
                embed.add_field(name="⚡ Offensive Efficiency (vs opposing starter handedness)", value="\n".join(offense_lines), inline=False)

            # Bullpen status
            bullpen_lines = []
            if sm.away_bullpen.fatigue_flag:
                bullpen_lines.append(
                    f"**{sm.away_team}**: {sm.away_bullpen.rested_key_relievers or '?'} rested | "
                    f"Bullpen xFIP {sm.away_bullpen.aggregate_xfip:.2f}" if sm.away_bullpen.aggregate_xfip else ""
                    f" | **{sm.away_bullpen.fatigue_flag.upper()}**"
                )
            if sm.home_bullpen.fatigue_flag:
                bullpen_lines.append(
                    f"**{sm.home_team}**: {sm.home_bullpen.rested_key_relievers or '?'} rested | "
                    f"Bullpen xFIP {sm.home_bullpen.aggregate_xfip:.2f}" if sm.home_bullpen.aggregate_xfip else ""
                    f" | **{sm.home_bullpen.fatigue_flag.upper()}**"
                )
            if bullpen_lines:
                embed.add_field(name="🫀 Bullpen Availability (72hr usage)", value="\n".join(bullpen_lines), inline=False)

            # Venue / Environment
            venue_lines = []
            if sm.venue.park_factor:
                venue_lines.append(f"Park Factor: **{sm.venue.park_factor:.0f}** (HR: {sm.venue.hr_factor:.0f})" if sm.venue.hr_factor else f"Park Factor: **{sm.venue.park_factor:.0f}**")
            ...
            # Python-calculated probabilities
            embed.add_field(
                name="📊 Python Win Probabilities (from Sabermetrics)",
                value=(
                    f"🚶 **{sm.away_team}**: {probs['away_prob']*100:.1f}%\n"
            ...
            embed.set_footer(text=f"Engine: {SURPLUS_MODEL} via Surplus | Kelly: f* = EV/(odds−1) × {kelly_frac} | Max 25%/bet | Python calc — no AI odds")

            print(f"[MLB CMD] Sending embed...", file=sys.stderr)
        ...
        try:
            ...
            print(f"[MLB CMD] Could not even send error message", file=sys.stderr)


@bot.command(name="mlbstatus")
async def cmd_mlb_status(ctx):
    """Show MLB bot configuration status."""
    gid = ctx.guild.id if ctx.guild else ctx.author.id
    threshold = get_ev_threshold(gid)
    kelly_frac = get_kelly_fraction(gid)
    bankroll = get_bankroll(gid)
...
async def on_command_error(ctx, error):
    ...
    try:
        ...
        print(f"[MLB Bot Error] Could not send error message to channel", file=sys.stderr)
